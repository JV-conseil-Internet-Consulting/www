...
self.StorageArea = require('./search.js').StorageArea;
self.MiniSearch = require('./search.js').MiniSearch;
...
var uniqBy = function uniqBy(xs, k) {
  return _toConsumableArray(new Map(xs.map(function (x) {
    return [x[k], x];
  })).values());
};
...
function _getDocuments() {
  _getDocuments = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee3(dataURL) {
    var _yield$fetch$then, _yield$fetch$then$pag, pages, _yield$fetch$then$doc, documents, siteData, docs;
    return _regeneratorRuntime().wrap(function _callee3$(_context3) {
      while (1) switch (_context3.prev = _context3.next) {
        case 0:
          _context3.next = 2;
          return fetch(dataURL).then(function (x) {
            return x.json();
          });
        case 2:
          _yield$fetch$then = _context3.sent;
          _yield$fetch$then$pag = _yield$fetch$then.pages;
          pages = _yield$fetch$then$pag === void 0 ? [] : _yield$fetch$then$pag;
          _yield$fetch$then$doc = _yield$fetch$then.documents;
          documents = _yield$fetch$then$doc === void 0 ? [] : _yield$fetch$then$doc;
          siteData = [].concat(_toConsumableArray(pages), _toConsumableArray(documents.map(function (doc) {
            if (doc.date) doc.date = new Date(doc.date);
            return doc;
          })));
          docs = uniqBy(siteData, 'url');
...
var miniSearch;
var lastEvent;
var storeEvent = function storeEvent(e) {
  lastEvent = e;
};
function search(_ref) {
  var term = _ref.data,
    _ref$ports = _slicedToArray(_ref.ports, 1),
    port = _ref$ports[0];
  var results = miniSearch.search(term, SEARCH_OPTIONS);
  port.postMessage(results.slice(0, 20));
}
_asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2() {
  var _yield$once, _yield$once$data, DATA_URL, STORAGE_KEY, INDEX_KEY, storage, indexData;
  return _regeneratorRuntime().wrap(function _callee2$(_context2) {
    while (1) switch (_context2.prev = _context2.next) {
      case 0:
        _context2.next = 2;
        return once(self, 'message');
      case 2:
        _yield$once = _context2.sent;
        _yield$once$data = _yield$once.data;
        DATA_URL = _yield$once$data.DATA_URL;
        STORAGE_KEY = _yield$once$data.STORAGE_KEY;
        INDEX_KEY = _yield$once$data.INDEX_KEY;
        // const storage = new StorageArea(STORAGE_KEY);
        storage = new self.StorageArea(STORAGE_KEY);
        _context2.next = 10;
      ...
      case 10:
        ...
        miniSearch = self.MiniSearch.loadJS(indexData, OPTIONS);
      ...
      case 16:
        ...
        miniSearch = new self.MiniSearch(OPTIONS);
        _context2.t0 = miniSearch;
        _context2.next = 21;
        return getDocuments(DATA_URL);
      case 21:
        _context2.t1 = _context2.sent;
        _context2.t0.addAll.call(_context2.t0, _context2.t1);
        if (lastEvent) search(lastEvent);
        self.removeEventListener('message', storeEvent);
        self.addEventListener('message', search);
        _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
          var oldKeys, _iteratorAbruptCompletion, _didIteratorError, _iteratorError, _iterator, _step, key;
          return _regeneratorRuntime().wrap(function _callee$(_context) {
            while (1) switch (_context.prev = _context.next) {
              case 0:
                ...
                _context.prev = 3;
                _iterator = _asyncIterator(storage.keys());
              case 5:
                _context.next = 7;
              ...
              case 29:
                ...
                return Promise.all(oldKeys.map(function (oldKey) {
                  return storage.delete(oldKey);
                }));
              case 31:
                _context.next = 33;
                return storage.set(INDEX_KEY, miniSearch.toJSON());
              case 33:
              case "end":
